...
class Clip3dTilesNew:
    """
    3D Tiles / B3DM / GLB XY 裁剪

     osgb2tiles5 / 3D Tiles 1.1：

        无 root.transform
        boundingVolume 已是 ECEF
        GLB node：rootNode(Y-up→Z-up) → enu(ENU→ECEF) → mesh(局部米)
        走完 node 矩阵后顶点已是 ECEF
    ...
    GLB：
        ...
        GLB POSITION (局部米)
            ...
            v
        GLTF Node Transform
        (rootNode: Z-up→Y-up, enu: ENU→ECEF)
            |
            v
        Y-up ECEF
            |
    ...
    def __init__(self, options: dict):
        ...
        self.ecef_to_wgs84 = Transformer.from_crs(
            "EPSG:4978",
            "EPSG:4979",
            always_xy=True,
        )
        ...
        # ENU 原点
        #
        # 会议中心：无 root.transform，用 boundingVolume ECEF 中心。
        # 医院：优先用 tileset root.transform 的 ECEF 平移。
        # ------------------------------------------------------------

        self.enu_origin = (
            self.resolve_enu_origin(
                self.clip_polygon_wgs84
            )
        )
    ...
    def resolve_enu_origin(
        self,
        polygon,
    ):

        tileset_file = (
            self.find_root_tileset_file()
        )
        ...
        return self.calculate_enu_origin(
            polygon
        )
    ...
    def read_primitive_mesh(
        ...
        position_accessor = attributes.get(
            "POSITION"
        )
    ...
    def extract_glb_meshes(
        ...
        # osgb2tiles5：
        # scene → rootNode(Z-up→Y-up) → enu(ENU→ECEF) → mesh(局部米)
        # 走完 node 矩阵后 positions 是 Y-up ECEF
    ...
    def b3dm_positions_to_enu(
        ...
        # ECEF
        #
        # 会议中心：extract 已乘 node 矩阵，顶点已是 ECEF。
        # 不要再做 Y-up → Z-up。
        # ------------------------------------------------------------
        ...
        # local_rtc
        #
        # POSITION 先从 glTF Y-up 转到 3D Tiles Z-up，
        # 再加 RTC_CENTER。
        #
        # transform = identity 时，结果就是模型本地 ENU。
        # ------------------------------------------------------------
    ...
    def local_vertices_to_enu(
        ...
        # glTF Y-up -> 3D Tiles Z-up
        #
        # 会议中心：extract 已乘 node 矩阵，
        # 结果是 Y-up ECEF，这里转到 Z-up ECEF。
        vertices = (
            self.y_up_to_z_up(
                vertices
            )
        ...
        # 会议中心：无 tileset transform，世界坐标已是 ECEF
        if self.is_ecef_xyz(
            world[0]
        ):
    ...
    def enu_vertices_to_local(
        ...
        # 会议中心：ENU -> Z-up ECEF -> Y-up ECEF
        # build_clipped_glb 使用扁平节点，Cesium 会再按 glTF Y-up 处理
        if self.is_identity_matrix(
            transform
        ):

            if self.b3dm_coordinate_mode in (
                "ecef",
                "auto",
            ):
    ...
    def process_glb(
        ...
        if relation == "outside":
            ...
            # 原样复制时保留 tileset 里原来的 Z-up ECEF 包围盒。
            # 不要用 GLB 烘焙后的 Y-up ECEF 去改 boundingVolume，
            # 否则放大 REPLACE 细化时会被误剔除。
            return (
                "copied",
                None,
            )
        ...
        for item in meshes:
            ...
            # ENU -> Y-up ECEF（与 extract 后的世界坐标一致）
            vertices_world = (
            ...
            world_matrix = item.get(
                "world_matrix"
            )

            # 写回 mesh 局部米制坐标，保留原来的 enu / rootNode。
            # 不能把 ECEF 烘焙进 float32 POSITION，否则放大后精度崩溃。
            if (
                world_matrix is not None
                and not self.is_identity_matrix(
                    world_matrix
                )
            ):
    ...
    def interpolate_z(
        ...
        weights = Clip3dTilesNew.barycentric_weights(
            x,
            y,
            a,
            b,
    ...
    def extract_polygons(
        ...
        if isinstance(
            ...
            for geom in geometry.geoms:
                ...
                result.extend(
                    Clip3dTilesNew.extract_polygons(
                        geom
                    )
                )
    ...
    # Bounding Volume
    #
    #  tileset.json 的 box 必须是 Z-up ECEF。
    # GLB 烘焙坐标是 Y-up ECEF，写包围盒前要先转过来。
    # ================================================================
    ...
    def build_clipped_glb(
        ...
        new_gltf = {
            ...
            # 保留原来的 rootNode / enu 节点。
            # POSITION 已写回局部米，放大时才能稳定显示。
            "scene":
                gltf.get(
                    "scene",
                    0,
        ...
        json_bytes = self.pad_chunk(
            json_bytes,
            b" ",
        )

        bin_bytes = self.pad_chunk(
            blob,
            b"\x00",
        )
...
if __name__ == "__main__":
    ...
    Clip3dTilesNew({
        ...
        #
        # ecef
        #    osgb2tiles5：包围盒 / GLB 已是 ECEF
        #
        # local_rtc
        #    osgb2tiles4：局部 ENU + tileset transform
        #
        # enu
        #   POSITION 已经是 ENU
        #
